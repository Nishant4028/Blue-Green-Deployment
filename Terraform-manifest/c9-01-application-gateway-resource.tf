...
# dummy rule setup
  frontend_port {
  name = "${azurerm_virtual_network.vnet.name}-feport-81"
  port = 81
}

frontend_port {
  name = "${azurerm_virtual_network.vnet.name}-feport-82"
  port = 82
}
http_listener {
  name                           = "${azurerm_virtual_network.vnet.name}-httplstn-81"
  frontend_ip_configuration_name = local.frontend_ip_configuration_name
  frontend_port_name             = "${azurerm_virtual_network.vnet.name}-feport-81"
  protocol                       = "Http"
}

http_listener {
  name                           = "${azurerm_virtual_network.vnet.name}-httplstn-82"
  frontend_ip_configuration_name = local.frontend_ip_configuration_name
  frontend_port_name             = "${azurerm_virtual_network.vnet.name}-feport-82"
  protocol                       = "Http"
}

  request_routing_rule {
  name                       = "dummy_rule_to_check_green_health"
  rule_type                  = "Basic"
  http_listener_name         = "${azurerm_virtual_network.vnet.name}-httplstn-81" # ✅ Reuse existing listener
  backend_address_pool_name  = local.backend_address_pool_name_app1
  backend_http_settings_name = local.http_setting_name_app1
  priority                   = 300
}
request_routing_rule {
  name                       = "dummy_rule_to_check_Blue_health"
  rule_type                  = "Basic"
  http_listener_name         = "${azurerm_virtual_network.vnet.name}-httplstn-82" # ✅ Reuse existing listener
